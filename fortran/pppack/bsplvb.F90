...
!  Parameters:
!
!    Input, real(wp) T(LEFT+JOUT), the knot sequence.  T is assumed to
...
!
!    Input, real(wp) X, the point at which the B-splines
!    are to be evaluated.
!
!    Input, integer LEFT, an integer chosen so that
!    T(LEFT) <= X <= T(LEFT+1).
!
!    Output, real(wp) BIATX(JOUT), with BIATX(I) containing the
...
!
  ...
  real(wp) biatx(jhigh)
  real(wp), save, dimension ( jmax ) :: deltal
  real(wp), save, dimension ( jmax ) :: deltar
!  real(wp), dimension ( jmax ) :: deltal
!  real(wp), dimension ( jmax ) :: deltar
  integer i
  integer index
  integer, save :: j = 1
  integer left
