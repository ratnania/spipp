...
!  Parameters:
!
!    Input, real(wp) T(N+K), the knot sequence.  T is assumed
!    to be nondecreasing.
!
!    Input, real(wp) BCOEF(N), B-spline coefficient sequence.
!
!    Input, integer N, the length of BCOEF.
!
!    Input, integer K, the order of the spline.
!
!    Input, real(wp) X, the point at which to evaluate.
!
!    Input, integer JDERIV, the order of the derivative to
!    be evaluated.  JDERIV is assumed to be zero or positive.
!
!    Output, real(wp) BVALUE, the value of the (JDERIV)-th
